...
namespace CodendOOP
{
    class SolverSettings
    {
        ...
        public double ReductionCondition { get; set; }
        public bool IncludeLineSearch { get; set; }
        ...
        public SolverSettings(InputReader filePath)
        {
            LoadDefault();
        ...
        private void LoadDefault()
        {
            ...
            AlphaMax = 0.2 * 0.05;                       // alpha for Armijo step limit globalization method (20% of twine length)  
            ReductionCondition = Math.Pow(10, -4);

            MinCatchBlock = 5;
            MinTowingSpeed = 1.0;

            IncludeLineSearch = false;
        ...
        private void LoadInput(string inpPath)
        {
            string[] names = { "IterMax", "ResidualTol", "DisplacementTol", "ResidualMax", "StiffnessTol",
                               ...
                               "LineSeachMax","AlphaMax","MinCatchBlock","MinTowingSpeed","IncludeLineSearch", "InitialShape"};
            ...
            foreach (var line in lines)
            {
                ...
                if (line.Contains(names[13]))
                    IncludeLineSearch = Convert.ToBoolean(Convert.ToInt16(parts[1]));

                if (line.Contains(names[14]))
                    InitialShape = parts[1];
